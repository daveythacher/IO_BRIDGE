...
SPI_Input::SPI_Input(uint8_t *vector, PIO pio, uint8_t pin_range_index, int clk_pin, int full_pin) {
    _vector = vector;
    _full_pin = full_pin;
    _clk_pin = clk_pin;
...
bool SPI_Input::read(uint8_t **buf) {
    bool result = false;
...
bool SPI_Input::hasFlowControl() {
    return true;
}

void SPI_Input::signalFull() {
